...
class ConstraintModel():
    ...
    def predict(self, X):
        ...
        if len(self._model) == 1:
            ...
            return (result.reshape(X_shape[:-1]), mu_sigma)
    ...
    def not_allowed(self, constraint_values):
        """Check whether `constraint_values` fulfills the specified limits.
        ...
        """
        if self._lb.size == 1:
            return (np.greater(self._lb, constraint_values)
                    & np.greater(constraint_values, self._ub))
