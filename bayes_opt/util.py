...
def acq_max(ac, gp, y_max, bounds, random_state, constraint=None, n_warmup=10000, n_iter=10, 
            y_max_params=None, y_abs_max=None, kind=None):
    ...
    if constraint is not None:
        def adjusted_ac(x):
            ...
            p_constraints, mu_sigma = constraint.predict(x.reshape(-1, bounds.shape[0]))
            values = -ac(x.reshape(-1, bounds.shape[0]), gp=gp, y_max=y_max, M=y_abs_max, 
                         mu_sigma=mu_sigma)
            if kind !='mei':
...
class UtilityFunction():
    ...
    kind: {'ucb', 'ei', 'mei', 'poi'}
        ...
        * 'mei' is the M-Expected Improvement method
    ...
    def __init__(self, kind='ucb', kappa=2.576, xi=0, kappa_decay=1, kappa_decay_delay=0, M=None):
        self.kappa = kappa
    ...
    def utility(self, x, gp, y_max, **params):
        ...
        if self.kind == 'mei':
            if self.M is not None:
                return self.mei(x, gp, y_max, M=self.M, **params)
            return self.mei(x, gp, y_max, **params)
        if self.kind == 'poi':
    ...
    def mei(x, gp, y_max, M, mu_sigma):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            mean_f, std_f = gp.predict(x, return_std=True)
