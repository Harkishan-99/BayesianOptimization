...
class TargetSpace():
    ...
    def _target_max(self, absolute=False, kind=None):
        ...
        if len(self.target[self.mask]) == 0:
            return None
        M = abs(self.target[self.mask]).max()
        if absolute:
            return M
        if kind != 'mei':
            return self.target[self.mask].max()
        adj_target = self.target - M*self._constraint.not_allowed(self._constraint_values)
        return adj_target.max()
